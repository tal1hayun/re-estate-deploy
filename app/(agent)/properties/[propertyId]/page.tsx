...
export default function PropertyPage({ params }: { params: Promise<{ propertyId: string }> }) {
  ...
  const [tab, setTab] = useState<'overview' | 'images' | 'price' | 'messages' | 'offers' | 'internal'>(
  ...
  async function copyPublicLink() {
    const activeLink = sharedLinks.find(l => l.is_active);
    let token: string;
    if (activeLink) {
    ...
    <div className="max-w-5xl mx-auto px-6 py-8">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {linkCopied ? '✓ הקישור הועתק בהצלחה' : 'העתק קישור ללקוח'}
          </button>
          {FB_POST_ENABLED && (
            <button
              onClick={openFbModal}
      ...
      <div className="flex gap-1 bg-gray-900 border border-gray-800 rounded-xl p-1 mb-6 w-fit flex-wrap">
        {([['overview', 'פרטים'], ['images', 'תמונות'], ['price', 'היסטוריית מחיר'], ['offers', 'הצעות'], ['messages', 'הודעות'], ['internal', 'פנימי 🔒']] as const).map(([key, label]) => (
          <button
            key={key}
            onClick={() => setTab(key)}
